...
//		var dateFont = (resourceMap.hasKey(dateFontOverride)) ? resourceMap[dateFontOverride] : rezFonts.DateFont;
	...
	function draw(dc) {
		...
		var day = now.day.format("%d"); // integer format
//		if (mYLine2 != null) {
...
//		var length = dc.getTextWidthInPixels(dateString, mFont);
		var x = 52; //(watchWidth / 2) - (length / 2);
//		var y = 74; //watchHeight/2-hoursHeight*0.8;
		var y = 130;
		
//		System.println("Height/2: "+watchHeight/2);
//		System.println("hoursHeight*0.8: "+hoursHeight*0.8);
//		System.println("xDate: "+x+" yDate: "+y);
		
		// Draw day.
		//dc.setColor(gMonoLightColour, Graphics.COLOR_TRANSPARENT);
		dc.setColor(DDColor, Graphics.COLOR_TRANSPARENT);
		dc.drawText(
			120,
			y, //watchHeight/2-hoursHeight*0.8,
			mFont,
			day,
			Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER
		);
		
		var xDifference = dc.getTextWidthInPixels(day, mFont);
...
//		x += dc.getTextWidthInPixels(day + " ", mFont);
		...
		// Draw month.
		//dc.setColor(gMonoDarkColour, Graphics.COLOR_TRANSPARENT);
		dc.setColor(monthColor, Graphics.COLOR_TRANSPARENT);
		dc.drawText(
			120+xDifference,
			y, //watchHeight/2-hoursHeight*0.8,
			mFont,
